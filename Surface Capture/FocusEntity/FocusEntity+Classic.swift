...
internal extension FocusEntity {
    ...
    func classicColorChanged(color: Material.Color) {
        print("**** CHANGING COLOR TO: \(color) ****")
        
        // Loop through each segment and set the new color
        for segment in segments {
            var modelMaterial: Material!
            
            if #available(iOS 15, macOS 12, *) {
                var mat = PhysicallyBasedMaterial()
                mat.baseColor = .init(tint: .black.withAlphaComponent(color.cgColor.alpha))
                //mat.emissiveColor = .init(color: color)
                //mat.emissiveIntensity = 0.5
                modelMaterial = UnlitMaterial(color: color)
                //modelMaterial = mat
            } else {
            ...
            segment.model?.materials[0] = modelMaterial
        }
    }
